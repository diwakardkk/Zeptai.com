...
export default function BlogCard({ commentCount = 0, post }: BlogCardProps) {
  return (
    <article className="group relative h-full overflow-hidden rounded-[30px] border border-border bg-card/92 shadow-[0_18px_60px_rgba(9,9,9,0.06)] transition duration-300 hover:-translate-y-1.5 hover:shadow-[0_24px_80px_rgba(9,9,9,0.1)]">
      ...
      <div className="flex h-[calc(100%-15rem)] flex-col px-6 pb-6 pt-2">
        ...
        <h3 className="mt-5 text-[1.4rem] font-bold leading-[1.2] tracking-[-0.03em] text-foreground transition group-hover:text-[#224bc3]">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {post.tags.slice(0, 3).map((tag) => (
            <span
              key={tag}
              className="rounded-full border border-border bg-card/90 px-2.5 py-1 text-xs font-medium text-muted-foreground"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="mt-auto pt-6">
          <div className="flex items-center justify-between rounded-[20px] border border-border bg-[linear-gradient(135deg,hsl(var(--card)),rgba(34,75,195,0.06),rgba(56,172,6,0.07))] px-4 py-3 text-sm text-muted-foreground">
            <div className="min-w-0">
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                Author
              </p>
              <p className="mt-1 truncate font-medium text-foreground">{post.author}</p>
            </div>
            <div className="flex items-center gap-2 rounded-full border border-[#224bc3]/20 bg-card/90 px-3 py-2 text-[#224bc3]">
