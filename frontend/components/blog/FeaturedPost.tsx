...
export default function FeaturedPost({ commentCount, post }: FeaturedPostProps) {
  return (
    <article className="mt-8 overflow-hidden rounded-[34px] border border-border bg-card/82 shadow-[0_28px_90px_rgba(9,9,9,0.08)] backdrop-blur-xl">
      <div className="grid grid-cols-1 lg:grid-cols-[1.08fr_0.92fr]">
        <div className="order-2 p-7 lg:order-1 lg:p-10 xl:p-12">
          ...
          <h2 className="mt-5 max-w-2xl text-3xl font-bold leading-[1.08] tracking-[-0.04em] text-foreground md:text-5xl">
            {post.title}
          </h2>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
            <div className="rounded-full border border-border bg-background px-4 py-2">
              ...
              <p className="mt-1 font-medium text-foreground">{post.author}</p>
            </div>
          ...
          <Link
            href={`/blog/${post.slug}`}
            className="mt-8 inline-flex items-center gap-2 rounded-full bg-foreground px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#224bc3]"
          >
