  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center px-4 py-8 sm:px-6">
      ...
      <section
        ...
        className="relative z-[71] w-full max-w-xl overflow-hidden rounded-3xl border border-[#224bc3]/20 bg-card/95 text-foreground shadow-[0_40px_100px_-40px_rgba(0,0,0,0.65)]"
      >
        <div className="border-b border-border bg-gradient-to-r from-[#38ac06]/10 via-card/80 to-[#224bc3]/10 px-5 py-4 sm:px-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                Custom Pricing
              </p>
              <h3 className="mt-1 text-xl font-bold text-foreground">Customize {planName}</h3>
            ...
            <button
              type="button"
              onClick={onClose}
              className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-border bg-card/90 text-muted-foreground transition hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#224bc3]/40"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
        ...
        <div className="space-y-6 px-5 py-5 sm:px-6 sm:py-6">
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="space-y-1.5 text-sm">
              <span className="font-medium text-foreground/85">Number of doctors</span>
              <input
                type="number"
                min={1}
                value={safeDoctors}
                onChange={(event) => setDoctors(Number(event.target.value))}
                className="h-12 w-full rounded-2xl border border-border bg-card px-4 text-base text-foreground shadow-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#38ac06]/40"
              />
            </label>

            <label className="space-y-1.5 text-sm">
              <span className="font-medium text-foreground/85">Number of reports</span>
              <input
                type="number"
                min={1}
                value={estimate.reportCount}
                onChange={(event) => setReports(Number(event.target.value))}
                className="h-12 w-full rounded-2xl border border-border bg-card px-4 text-base text-foreground shadow-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#224bc3]/40"
              />
            </label>
          </div>

          <div className="rounded-2xl border border-[#224bc3]/20 bg-card/90 p-4">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <p className="mt-1 text-lg font-bold text-foreground">{estimate.estimatedCostText}</p>
              </div>
            </div>
            <p className="mt-3 text-xs text-muted-foreground">{estimate.note}</p>
          </div>

          <div className="grid gap-3 rounded-2xl border border-border bg-background/80 p-4 text-xs text-muted-foreground sm:grid-cols-3">
            <span className="inline-flex items-center gap-1.5 rounded-xl border border-border bg-card px-2.5 py-2">
        ...
        <div className="flex flex-col gap-2 border-t border-border px-5 py-4 sm:flex-row sm:justify-end sm:px-6">
          <button
            type="button"
            onClick={onClose}
            className="inline-flex h-11 items-center justify-center rounded-full border border-border px-5 text-sm font-semibold text-foreground/85 transition hover:bg-muted"
          >
            Cancel
          </button>
          <button
