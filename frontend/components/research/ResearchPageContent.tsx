...
const publicationsData: Publication[] = [
  ...
  },
  {
    ...
    authors: "R. Smith, S. Johnson, J. Doe",
...
export default function ResearchPageContent() {
  ...
  const getStatusClasses = (status: Publication["status"]) => {
    switch (status) {
      ...
      case "Accepted":
        return "border-emerald-300/40 bg-emerald-50 text-emerald-700 dark:bg-emerald-950/20 dark:text-emerald-400";
      case "Preprint":
        return "border-blue-300/40 bg-blue-50 text-blue-700 dark:bg-blue-950/20 dark:text-blue-400";
      case "Under Review":
        return "border-amber-300/40 bg-amber-50 text-amber-700 dark:bg-amber-950/20 dark:text-amber-400";
      default:
        return "border-slate-300/40 bg-slate-50 text-slate-700 dark:bg-slate-900 dark:text-slate-400";
    }
  };
  ...
  const getToneClasses = (tone: string) => {
    switch (tone) {
      case "green":
        return "border-[#38ac06]/20 bg-[#38ac06]/10 text-[#2f8f07]";
      case "amber":
        return "border-amber-300/45 bg-amber-500/10 text-amber-700 dark:text-amber-400";
      case "blue":
        return "border-[#224bc3]/20 bg-[#224bc3]/10 text-[#224bc3]";
      case "slate":
      default:
        return "border-slate-300/60 bg-slate-500/10 text-slate-700 dark:text-slate-400";
    }
  };
  ...
  return (
    <section className="relative overflow-hidden pb-20 pt-28 md:pb-24 md:pt-32">
      ...
      <div className="pointer-events-none absolute inset-0 -z-10">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(rgba(34,75,195,0.035)_1px,transparent_1px),linear-gradient(90deg,rgba(34,75,195,0.035)_1px,transparent_1px)] bg-[size:62px_62px] opacity-25" />
      </div>
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-14 px-4 sm:px-6 lg:gap-18 lg:px-8">
        ...
        <section className="grid gap-8 lg:grid-cols-2 lg:items-stretch">
          ...
          <article className="rounded-[2.2rem] border border-white/70 bg-white/80 dark:bg-card/70 p-7 shadow-[0_24px_64px_-40px_rgba(20,32,72,0.3)] backdrop-blur-sm sm:p-9 flex flex-col justify-between">
            <div>
        ...
        <section id="publications" className="rounded-[2rem] border border-white/75 bg-white/84 dark:bg-card/40 p-6 shadow-[0_28px_64px_-46px_rgba(20,32,72,0.44)] backdrop-blur-sm sm:p-8">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {filteredPublications.map((paper) => (
              <article
                key={paper.doi}
                className="rounded-[1.8rem] border border-white/70 bg-white/90 dark:bg-card/80 p-5 shadow-[0_20px_48px_-36px_rgba(20,32,72,0.22)] flex flex-col justify-between"
              >
                <div>
